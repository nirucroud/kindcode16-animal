...
function go(id){document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById(id).classList.add('active');scrollTo(0,0);if(id==='quiz')startQuiz()}document.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>go(b.dataset.go));
...
function showResult(){let max=score.indexOf(Math.max(...score));let t=types[max];resultTitle.textContent=t.name;resultSub.textContent=`${t.id}｜${t.sub}`;resultImg.src='assets/results/'+t.img;go('result')}
const books=['animal1.jpeg','animal2.jpeg','mystery1.jpeg','mystery2.jpeg'];function showBook(i){bookImg.src='assets/book/'+books[i]}window.showBook=showBook;
